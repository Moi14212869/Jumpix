...
let keyboardLayout = localStorage.getItem("keyboardLayout") || "zqsd";
...
function checkObjectives() {
    // Déblocage des skins existants
    if (dead >= 1000) localStorage.setItem("skin_000000", "1");   // Noir après 1000 morts
    if (kill >= 500) localStorage.setItem("skin_FF0000", "1");   // Rouge après 500 kills
    // Nouveau : skin spécial après beaucoup de parties jouées
...
function createPlatform(scene, x, y, widthInPx, heightInPx = 40, color = 0xA0522D) {
    ...
    for (let row = 0; row < blocksPerCol; row++) {
        for (let col = 0; col < blocksPerRow; col++) {
            ...
            // ❄️ Neige sur le dessus (uniquement World2 et première rangée)
            if (isWorld2 && row === 0) {
                gfx.fillStyle(0xffffff, 1);
                gfx.fillRect(0, 0, tileSize, 8); // épaisseur de neige
            }
...
function createSnow(scene) {
    ...
    particles.setDepth(-10);      // arrière-plan
    particles.setScrollFactor(0); // fixe à l'écran
}
...
class LoadingScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.on('complete', () => {
		 ...
		 create() {
        // Lancement du menu après un petit délai
...
backButton.on("pointerdown", () => {
  ...
  // Si le joueur touche par le haut (avec une petite marge de tolérance)
  if (verticalDiff < 10 && player.body.velocity.y > 0) {
    // Le joueur a sauté sur le carré rouge
	kill++;
  ...
  if (p.body.touching.down && this.onIce) {
        ...
        if (!this.sliding) {
            if (left || right) {
                ...
                this.sliding = true;
                this.slideDir = this.inheritedSlideDir; // direction héritée
            }
			        if (this.sliding) {
    ...
    // --- Reset flag glace à la fin du frame ---
    this.onIce = false;
...
jumpPlayer() {
    if (this.jumpCount < 2) {
        ...
        let dir = 0;
        if (this.player.body.velocity.x > 0) dir = 1;
        else if (this.player.body.velocity.x < 0) dir = -1;

        // --- Direction par défaut (horaire) ---
        if (dir === 0) dir = 1;

        // --- DOUBLE SAUT = DEMI ROTATION ---
        if (this.jumpCount === 1) {
            this.tweens.add({
                targets: this.player,
        ...
        // --- Si saut depuis la glace, mémoriser la direction ---
        if (this.onIce) {
            if (this.cursors.left.isDown || this.keys.left.isDown || this.movingLeft) {
                this.inheritedSlideDir = -1;
            } else if (this.cursors.right.isDown || this.keys.right.isDown || this.movingRight) {
                this.inheritedSlideDir = 1;
            } else {
                this.inheritedSlideDir = this.slideDir;
            }
        }
...
class World1 extends Phaser.Scene {
    ...
    create() {
		...
		this.coinText = this.add.text(780, 20, `💰 ${playerCoins}`, {
    		fontSize: "28px",
    		color: "#ffff00"
		}).setOrigin(1, 0); // aligné à droite
		const MenuButton = this.add.text(5, 5, "←", {
            fontSize: "24px",
            color: "#ffffff",
            backgroundColor: "#00BFFF",
            padding: { x: 7, y: 4 },
        ...
        niveaux1.forEach(niv => {
            ...
            yPos1 += 60; // écart vertical entre boutons
        });
		        const niveaux2 = [
            { nom: "➡ Level 5", scene: "Level5" },
			{ nom: "➡ Level 6", scene: "Level6" },
			{ nom: "➡ Level 7", scene: "Level7" },
			{ nom: "➡ Level 8", scene: "Level8" }
        ];
        ...
        niveaux2.forEach(niv => {
            ...
            yPos2 += 60; // écart vertical entre boutons
        });
    }
}
/*======================================================
...
class SettingsScene extends Phaser.Scene {
    ...
    create() {
		...
		const layoutText = this.add.text(50, 200, "", {
  fontSize: "24px",
  fill: "#ffffff"
});
...
keyboardBtn.on("pointerdown", () => {
    keyboardLayout = keyboardLayout === "zqsd" ? "wasd" : "zqsd";
...
class CreditsScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Créer un conteneur pour tous les textes
        ...
        // Événement clic partout sur la scène
    ...
    update(time, delta) {
        // Faire défiler les crédits vers le haut
        this.creditContainer.y -= this.scrollSpeed * (delta / 1000);

        // Si le dernier texte est passé au-dessus de l'écran, revenir au menu automatiquement
...
class Stats extends Phaser.Scene {
    ...
    create() {
        ...
        const statsData = [
            ...
            { label: "Deaths ☠️", value: deaths },
            { label: "Kills 🔪", value: kills },
            { label: "Parties Played 🎮", value: parties }
        ];
        ...
        // Bouton retour
        const backButton = this.add.text(10, 10, "←", {
            fontSize: "24px",
            color: "#ffffff",
            backgroundColor: "#00BFFF",
            padding: { x: 10, y: 5 },
    ...
    },
    {
      ...
      const isSelected = colorPlayer === obj.color;
        ...
        select.on("pointerdown", () => {
            ...
            // Mettre à jour le texte du bouton pour refléter la sélection
